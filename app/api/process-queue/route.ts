...
export async function POST() {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: `Processed ${processed} lead generation jobs`,
      processed
    });
...
async function sendLeadResultsEmail(email: string, refCompany: string, companies: {
  company_name: string;
  website?: string;
  industry?: string;
  company_size?: string;
  location?: string;
  why_chosen?: string;
  company_contacts?: { contact_name: string; contact_email?: string; contact_role?: string; }[];
}[]) {
